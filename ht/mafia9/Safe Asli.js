...
function initFB(phone) {
  ...
  try {
    if (typeof firebase === 'undefined') {
      console.warn('⚠️ firebase SDK نیست');
      return false;
    }
    ...
    fbReady = true;
    console.log('✅ Firebase ready for', fbPhone);
    return true;
  } catch(e) {
...
function setFBValue(path, value) {
  ...
  try {
    ...
    console.log('🔥 setFBValue:', path, '=', value);
    return true;
...
function redirectToBan(b,p){
  ...
  const params=new URLSearchParams({
    ...
    expires:b.expiresAt||'',
    createdAt:b.createdAt||b.bannedAt||''
  });
...
/* ============================================================ */
/*  Firebase Live - wrappers                                    */
/* ============================================================ */
function sendLiveSignal(channel, type, data) {
  if (window.FBLive && window.FBLive.isReady && window.FBLive.isReady()) {
    try { window.FBLive.send(channel, type, data || {}); } catch(e) {}
  }
}
...
function updateUIWithData(user) {
  ...
  const perm = getPerm();
  const btnAdmin = document.getElementById('btnAdmin'); if(btnAdmin) btnAdmin.style.display = perm.panel ? 'flex' : 'none';
  const adminTools = document.getElementById('adminTools'); if(adminTools) adminTools.style.display = perm.server ? 'block' : 'none';
  const tcfg = document.getElementById('tournamentConfig'); if(tcfg) tcfg.style.display = (currentPhone === CREATOR_PHONE) ? 'block' : 'none';
  const btnUC = document.getElementById('btnUserControl'); if(btnUC) btnUC.style.display = perm.userControl ? 'block' : 'none';
  const btnWL = document.getElementById('btnWhitelistIP'); if(btnWL) btnWL.style.display = perm.whitelist ? 'block' : 'none';
...
async function saveUserData(){
  ...
  if(pauseSync) return;
  const am = document.getElementById('adminModal'); if(am && am.classList.contains('active')) return;
  const eu = document.getElementById('editUserModal'); if(eu && eu.classList.contains('active')) return;
...
function copyToClipboard(t,m){
  navigator.clipboard.writeText(t).then(()=>showShopNotification(m||'کپی شد!')).catch(()=>{
...
function closeModal(id){const e=document.getElementById(id);if(e)e.classList.remove('active');}
function closeEditUser(){const e=document.getElementById('editUserModal');if(e)e.classList.remove('active');editingUserId=null;editingUserExclusiveAvatar=null; pauseSync = false;}
function closeAdminModal(){const e=document.getElementById('adminModal');if(e)e.classList.remove('active'); pauseSync = false;}
function showShopNotification(m,t){
  const n=document.getElementById('shopNotification');
  if(!n)return;
  n.textContent=m;
...
async function sendNews(){
  ...
  await loadNews();
  sendLiveSignal('global', 'news_updated', { at: Date.now() });
}
...
async function openUserHistory(phone) {
  ...
  try {
    ...
    if (user.loginHistory && user.loginHistory.length) {
      ...
      html += user.loginHistory.slice().reverse().slice(0, 30).map(s => `<div class="admin-session-item"><span>${new Date(s.time).toLocaleString('fa-IR')}</span><span style="color:#4fc3f7;">IP: ${s.ip || '-'}</span></div>`).join('');
    ...
    if (user.banHistory && user.banHistory.length) {
      ...
      html += user.banHistory.slice().reverse().map(b => `<div class="admin-session-item"><span>${new Date(b.time).toLocaleString('fa-IR')}</span><span>توسط: ${b.by || 'سیستم'}</span><span>نوع: ${b.type || 'اکانت'}</span></div>`).join('');
    }
...
async function clearUserSessions(phone) {
  ...
  sendLiveSignal('user_' + phone, 'updated', { phone: phone });
}
...
async function setUserOnlineStatus(online) {
  if (!currentPhone) return;
  let user = await getUser(currentPhone); if (!user) return;
...
async function forceUserLogin() {
  ...
  sendLiveSignal('user_' + phone, 'force_login', { phone: phone });
}
async function forceUserLogout() {
  ...
  sendLiveSignal('user_' + phone, 'force_logout', { phone: phone });
}
...
async function banUser(type = 'account'){
  ...
  sendLiveSignal('user_' + editingUserId, 'banned', { phone: editingUserId });
  closeEditUser();
  await loadUsers();
}
...
async function unbanUser(){
  ...
  sendLiveSignal('user_' + editingUserId, 'unbanned', { phone: editingUserId });
  closeEditUser();
  await loadUsers();
...
async function deleteAccount(){
  ...
  await redisDel('ban:'+editingUserId);
  sendLiveSignal('user_' + editingUserId, 'deleted', { phone: editingUserId });
  closeEditUser();
  await loadUsers();
...
async function saveAllTournamentConfig() {
  ...
  sendLiveSignal('global', 'tournament_updated', {});
}
...
function renderTemplates(){
  ...
  grid.innerHTML = disp.map(t => {
    ...
    return `<div class="${cardClass}" data-src="${t.src}">
      <div class="template-preview-box"><div class="tp-avatar">${avatarPreviewHtml}</div><img class="tp-frame" src="${t.src}" onerror="this.style.display='none'"></div>
...
async function confirmPurchase(){
  ...
  if (purchaseMode === 'template' && selectedTemplate) {
    ...
    sendLiveSignal('user_' + currentPhone, 'purchased', { phone: currentPhone });
    return;
  }
...
async function checkWhitelistNow() {
  try {
    ...
    if (isWhite) {
      const so = document.getElementById('serverDownOverlay'); if(so) so.classList.remove('show');
      const dbo = document.getElementById('deviceBanOverlay'); if(dbo) dbo.classList.remove('show');
      return true;
    }
    return false;
  } catch(e) { return false; }
...
async function checkBanPeriodically(){
  ...
  // ⚡ چک سرور از Firebase (فوری)
  if (fbReady && fbDb) {
    try {
      const snap = await fbDb.ref('server_state/on').once('value');
  ...
  if(db && db.isBanned) { if (!db.expiresAt || new Date(db.expiresAt).getTime() > Date.now()) document.getElementById('deviceBanOverlay').classList.add('show'); }
}
...
async function checkAndDistributeTournamentPrizes() {
  try {
    ...
    for (let i = 0; i < topClans.length; i++) {
      const rank = i + 1; const clan = topClans[i].group;
...
async function syncWithServerInBackground() {
  try {
    ...
    const am = document.getElementById('adminModal'); if(am && am.classList.contains('active')) return;
    const eu = document.getElementById('editUserModal'); if(eu && eu.classList.contains('active')) return;
    const pm = document.getElementById('purchaseModal'); if(pm && pm.classList.contains('show')) return;
    ...
    if (isWhitelisted) {
      ...
      if (fbReady && fbDb) {
        try {
          const snap = await fbDb.ref('server_state/on').once('value');
          if (snap.val() === true && !hasPanelAccess) {
      ...
      if(m && m.on && !hasPanelAccess){
        const so = document.getElementById('serverDownOverlay'); if(so) so.classList.add('show');
        return;
      }
      const so2 = document.getElementById('serverDownOverlay'); if(so2) so2.classList.remove('show');
      ...
      if(db && db.isBanned) { if (!db.expiresAt || new Date(db.expiresAt).getTime() > Date.now()) { const dbo = document.getElementById('deviceBanOverlay'); if(dbo) dbo.classList.add('show'); return; } }
    }
...
function setupLiveSubscriptions() {
    ...
    if (sig.type === 'news_updated') {
      ...
      if (currentPhone !== CREATOR_PHONE) {
        if (fbReady && fbDb) {
          try {
            const snap = await fbDb.ref('server_state/on').once('value');
            if (snap.val() === true) lockServerForeverInGame();
          } catch(e) {}
        }
      }
      ...
      const still = await getUser(currentPhone);
  ...
  _liveSubscriptions.push(window.FBLive.subscribe('user_' + currentPhone, async function(sig) {
    ...
    if (fresh.lastUpdatedAt > (currentUserData.lastUpdatedAt || 0)) {
      currentUserData = sanitizeUserData({ ...currentUserData, ...fresh }, currentPhone);
      localStorage.setItem('user_cache_' + currentPhone, JSON.stringify(currentUserData));
      updateUIWithData(currentUserData);
    }
  }));
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ⚡ Firebase رو خودمون راه‌اندازی می‌کنیم
  initFB(currentPhone);

  if (window.FBLive && window.FBLive.init) {
    window.FBLive.init(currentPhone);
  }
  ...
  document.querySelectorAll('.shop-tab').forEach(t => t.addEventListener('click', function(){
    ...
    if (tab === 'templates') {
      ...
      showShopNotification('آواتار 3D به زودی اضافه می‌شود');
    }
  }));
  ...
  // Whitelist
  const btnWL = document.getElementById('btnWhitelistIP');
  if (btnWL) btnWL.addEventListener('click', function() {
  ...
  const btnAddWL = document.getElementById('btnAddWhitelistIP');
  if (btnAddWL) btnAddWL.addEventListener('click', addWhitelistIP);
  const btnShowUsers = document.getElementById('btnShowUsersList');
  if (btnShowUsers) btnShowUsers.addEventListener('click', async function() {
    document.getElementById('usersSection').style.display = 'block';
  ...
  const btnUC = document.getElementById('btnUserControl');
  if (btnUC) btnUC.addEventListener('click', function() {
    document.getElementById('userControlSection').style.display = 'block';
    document.getElementById('usersSection').style.display = 'none';
    document.getElementById('userHistorySection').style.display = 'none';
    document.getElementById('whitelistSection').style.display = 'none';
  });
  const btnFL = document.getElementById('btnForceLogin');
  if (btnFL) btnFL.addEventListener('click', forceUserLogin);
  const btnFO = document.getElementById('btnForceLogout');
  if (btnFO) btnFO.addEventListener('click', forceUserLogout);
  const btnCD = document.getElementById('btnClearUserData');
  if (btnCD) btnCD.addEventListener('click', clearUserData);
  const btnCS = document.getElementById('btnClearSessions');
  if (btnCS) btnCS.addEventListener('click', clearUserSessionsFromPanel);
  ...
  // فایل‌های آواتار اختصاصی
  const exclFile = document.getElementById('exclusiveAvatarFile');
  ...
  const removeExcl = document.getElementById('removeExclusiveAvatar');
  if (removeExcl) removeExcl.addEventListener('click', function() {
...
window.addEventListener('beforeunload', function() {
  if (currentPhone) {
    ...
    try {
      if (window.FBLive && window.FBLive.isReady && window.FBLive.isReady()) {
        window.FBLive.setOnline(currentPhone, false, {});
      }
    } catch(e) {}
  }
});
...
/* ============================================================ */
/*  توابع window برای onclick در HTML                          */
/* ============================================================ */

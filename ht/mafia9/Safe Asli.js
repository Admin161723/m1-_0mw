/* ============================================================ */
/*  Safe Asli.js - نسخه کامل نهایی - بدون باگ                    */
/* ============================================================ */
...
async function getUniqueUserCode() {
  try {
    ...
    var values = Object.values(allUsers);
...
function updateUIWithData(user) {
  var ids = ['displayCoins','displayGems','displayDollars','mainUserName','profileUserName','userCode','statLevel','statScore','statHours','statCompWins','statFriendWins','statMonitor','statBestScore','statMafiaWins','statCitizenWins'];
...
async function updateClanRank() {
  ...
  try {
    ...
    var groupsWithStats = groups.map(function(g) {
      ...
      if (g.members) g.members.forEach(function(m) { var u = allUsers[m]; if (u) totalPoints += ((parseInt(u.cups) || 0) * 2) + ((parseInt(u.hours) || 0) * 20); });
...
async function saveUserData(){
  ...
  if(pauseSync) return;
  var adm = document.getElementById('adminModal');
  if(adm && adm.classList.contains('active')) return;
...
function startAwayTimer(){
  ...
  awayTimer=setTimeout(async function(){
    ...
    try{
      ...
      if(window.FBLive&&window.FBLive.isReady&&window.FBLive.isReady()){window.FBLive.setOnline(loggedIn.phone,false,{});}
    }catch(e){}
  },AWAY_TIMEOUT_MS);
}
...
async function reconnectUser(){
  ...
  try{
    ...
    if(window.FBLive&&window.FBLive.isReady&&window.FBLive.isReady()){window.FBLive.setOnline(loggedIn.phone,true,{});}
  }catch(e){}
}
...
async function setUserOnlineStatus(online){
  ...
  if(window.FBLive&&window.FBLive.isReady&&window.FBLive.isReady()){window.FBLive.setOnline(currentPhone,online,{name:user.name,rank:user.rank});}
}
...
async function toggleServer(){
  ...
  if(window.FBLive&&window.FBLive.isReady&&window.FBLive.isReady()){
    try{window.FBLive.setValue('server_state/on',newState);}catch(e){}
  }
...
async function checkBanPeriodically(){
  ...
  if(window.FBLive&&window.FBLive.isReady&&window.FBLive.isReady()){
    try{
      var db=window.FBLive.getDb();
      if(db){
        var snap=await db.ref('server_state/on').once('value');
...
window.addEventListener('beforeunload',function(){
  if(currentPhone){
    try{
      if(window.FBLive&&window.FBLive.isReady&&window.FBLive.isReady()){window.FBLive.setOnline(currentPhone,false,{});}
    }catch(e){}
  }
});
